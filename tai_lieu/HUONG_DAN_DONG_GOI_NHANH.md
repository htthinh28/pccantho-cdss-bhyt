...
2. Chạy **chỉ bản Portable** (nhanh hơn bản đầy `desktop:build:win` vì không tạo NSIS):
   ...
   Hoặc **một click** (ghi ra `packaging/out_portable/`): chạy `packaging/build_portable_full.bat`.

   `desktop:export` trước khi đóng gói đã gồm: `tai_lieu:prepare`, chỉ mục huấn luyện, chỉ mục `tai_nguyen/`, rồi `expo export`. Export nhanh không cần chỉ mục: `npm run desktop:export:light`.
