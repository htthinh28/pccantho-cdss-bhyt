/**
 * ============================================================
 * FILE: danh_muc_byt/10_ma_doi_tuong_kham/10_ma_doi_tuong_kham_dq3276.jsx
 * MỤC ĐÍCH: Quản lý DM Mã đối tượng đến KCB (QĐ 3276/QĐ-BYT)
...
const STORAGE_KEY = '@DM_BYT_PL10_V1';

const PhuLuc10MaDoiTuong = () => {
  ...
  const loadData = async () => {
    try {
      ...
      console.error('Lỗi tải dữ liệu PL10:', error);
    }
  };
  ...
  const autoSaveToStorage = async (newData, newFields) => {
    try {
      ...
      console.error('Lỗi Auto-save PL10:', error);
    }
  };
  ...
  const handleDownloadTemplate = () => {
    Alert.alert(
      "File Mẫu", 
      "File Excel cần các cột: MA_DOITUONG_KCB, Trường hợp, Quy định, MỤC_HUONG. Bạn có thể tự thêm cột bằng nút [+ Thêm Cột] trước khi Import."
    );
  };
  ...
  const handleImportExcel = async () => {
    try {
      ...
      const mergedData = [...data, ...parsedData.map((item, idx) => ({
        id: item['MA_DOITUONG_KCB']?.toString() || item['Mã đối tượng']?.toString() || Date.now().toString() + idx,
        ma_doi_tuong: item['MA_DOITUONG_KCB']?.toString() || item['Mã đối tượng']?.toString() || '',
        truong_hop: item['Trường hợp'] || '',
        quy_dinh: item['Quy định'] || '',
        muc_huong: item['MỤC_HUONG'] || item['Mức hưởng'] || '',
        ...customFields.reduce((acc, field) => ({ ...acc, [field]: item[field] || '' }), {})
      }))].filter(item => item.ma_doi_tuong !== ''); 
      
      // Lọc trùng theo Mã đối tượng
      const uniqueData = Array.from(new Map(mergedData.map(item => [item.ma_doi_tuong, item])).values());
  ...
  const handleExportExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "10_MaDoiTuongKCB");
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <View style={styles.row}>
        ...
        {isEditing ? (
          <React.Fragment>
            <TextInput style={[styles.cellEdit, { flex: 1.5 }]} value={editForm.ma_doi_tuong} onChangeText={(t) => setEditForm({...editForm, ma_doi_tuong: t})} />
            <TextInput style={[styles.cellEdit, { flex: 4 }]} value={editForm.truong_hop} onChangeText={(t) => setEditForm({...editForm, truong_hop: t})} multiline />
            <TextInput style={[styles.cellEdit, { flex: 2 }]} value={editForm.quy_dinh} onChangeText={(t) => setEditForm({...editForm, quy_dinh: t})} multiline />
            <TextInput style={[styles.cellEdit, { flex: 2 }]} value={editForm.muc_huong} onChangeText={(t) => setEditForm({...editForm, muc_huong: t})} multiline />
          ...
          <React.Fragment>
            <Text style={[styles.cell, { flex: 1.5, color: '#D84315', fontWeight: 'bold' }]}>{item.ma_doi_tuong}</Text>
            <Text style={[styles.cell, { flex: 4, fontWeight: '500' }]}>{item.truong_hop}</Text>
            <Text style={[styles.cell, { flex: 2, color: '#555', fontStyle: 'italic' }]}>{item.quy_dinh}</Text>
            <Text style={[styles.cell, { flex: 2, color: '#1565C0', fontWeight: 'bold' }]}>{item.muc_huong}</Text>
            {customFields.map(field => (
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>PHỤ LỤC 10 - MÃ ĐỐI TƯỢNG ĐẾN KCB (QĐ 3276)</Text>
      ...
      <TextInput 
        style={styles.search} 
        placeholder="Tra cứu mã đối tượng, trường hợp..." 
        value={searchQuery} 
      ...
      <ScrollView horizontal>
        <View style={{ minWidth: '100%' }}>
          <View style={styles.headerRow}>
            ...
            <Text style={[styles.headerCell, { flex: 1.5 }]}>MÃ ĐỐI TƯỢNG</Text>
            <Text style={[styles.headerCell, { flex: 4 }]}>TRƯỜNG HỢP</Text>
            <Text style={[styles.headerCell, { flex: 2 }]}>QUY ĐỊNH</Text>
            <Text style={[styles.headerCell, { flex: 2 }]}>MỨC HƯỞNG</Text>
            {/* Cột Custom */}
            {customFields.map(field => (
...
export default PhuLuc10MaDoiTuong;